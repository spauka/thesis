...
\begin{figure}
  ...
  \textsc{BPP} includes all problems that a classical computer can solve efficiently (including
  everything that can be calculated in polynomial time), \textsc{BQP} is all problems a quantum computer can solve
  efficiently (which includes everything a classical computer can do). Finally, problems that scale exponentially
  with input size, labelled \textsc{EXP}, is (almost) all other problems that are computable. The class
  \textsc{NP} is also included on this figure as it is one that often comes up in the context of complexity,
...
figure~\ref{fig:complexity}. To emphasize this point, note that although \textsc{BQP} is larger than \textsc{P}
or \textsc{BPP}, it certainly does not inclose all problems, especially those in \textsc{EXP}. Although a
